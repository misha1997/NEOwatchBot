// Fullscreen PixiJS view of Jupiter's moon system.
//
// This is a separate immersive mode for the existing `/planetarium/jupiter` page.
// The inline SVG widget (`#moon-system` in Jupiter.js) is left untouched; this
// component is opened from a fullscreen button inside that widget's controls.
//
// Data model (mirrors the backend `services/jupiter.py` / the SVG widget):
//   moons[]: { name, name_uk, group, a_km, period_d, e, i_deg, prograde,
//             m0_deg, diameter_km }
// Coordinate model (same as Jupiter.js):
...
//        ├─ jupiterDisc (Graphics) // true-scale planet disc
...
// ---------------------------------------------------------------------------
// Constants shared with the SVG widget in Jupiter.js
// ---------------------------------------------------------------------------
...
const GROUPS = ["ring", "galilean", "himalia", "ananke", "carme", "pasiphae"];
...
// every zoom level (e.g. Io really orbits at ~6× Jupiter's radius, not ~2.4×
...
// photo of that moon. Pushed high so photos only appear once Jupiter itself
// is drawn at a comparable-or-larger screen size — keeps each photo smaller
// than the planet (moons are genuinely ≪ Jupiter on the true-size scale).
...
// moon sizes and guarantees photo < Jupiter disc at every zoom.
const PHOTO_MULT = 5;
const PHOTO_MIN_PX = 26;
const PHOTO_MAX_PX = 90;
// Planet photo cross-fade: as you zoom in, the flat Jupiter disc dissolves into
// the real planet image (/planets/Jupiter.png). Pushed high so the photo only
...
// same scale as the Jupiter disc (see geo). Clamped on screen to
...
const DOT_MAX_PX = 14;
// Jupiter disc: drawn at true scale but clamped to a min on-screen radius so
...
// Helpers for formatting numeric values (same helpers as Jupiter.js).
function spacer(n) {
...
// the interior — so hovering Jupiter or the inner system doesn't trigger every
...
// position & rotation). "kepler" ⇒ real eccentricity ellipse with Jupiter at
// the focus; "circle" ⇒ simplified circle of radius = semi-major axis centred
// on Jupiter. Also makes the ring interactive (annular hit area, pointer
...
// Component
// ---------------------------------------------------------------------------
export default function JupiterMoonSystemFullscreen({ onClose }) {
  ...
  const discRef = useRef(null); // Jupiter disc Graphics (redrawn each LOD pass)
  ...
  const discPhotoRef = useRef(null); // Jupiter photo Sprite (cross-fades in on zoom)
  const discPhotoStateRef = useRef({ loaded: false, loading: false, texMax: 1 });
  const planetGlowRef = useRef(null); // soft halo behind the Jupiter photo
  ...
  // the SAME true-size scale as the Jupiter disc: dotR = rMax * (bodyRadius_km
  // / aMax), mirroring discR = rMax * (JUPITER_RADIUS_KM / aMax). So moons are
  // always drawn smaller than the planet (Ganymede ≈ 1/26 of Jupiter), and
  ...
  const geo = useMemo(() => {
    ...
    return moons.map((m) => {
      ...
      const isGalilean = ["io", "europa", "ganymede", "callisto"].includes(m.name.toLowerCase());
      const hash = getNameHash(m.name);
      const rot = isGalilean ? 0 : (Math.abs(hash) % 360) * (Math.PI / 180);
      const flipX = isGalilean ? 1 : (hash & 1 ? -1 : 1);
      const flipY = isGalilean ? 1 : (hash & 2 ? -1 : 1);
      const scaleMult = isGalilean ? 1.0 : 0.85 + (Math.abs(hash % 100) / 100) * 0.3;
      
      const tints = [0xffffff, 0xf2ebe1, 0xe1e6ec, 0xebebeb, 0xf6ebd8];
      const tint = isGalilean ? 0xffffff : tints[Math.abs(hash) % tints.length];
  ...
  const discR = useMemo(() => {
    ...
    return SYS.rMax * Math.pow(JUPITER_RADIUS_KM / aMax, ORBIT_POWER);
  }, [moons]);
  ...
  useEffect(() => {
    ...
    // Jupiter disc (true scale, drawn once here; the ticker redraws it each
    ...
    // Jupiter photo sprite: cross-fades in over the flat disc as you zoom in.
    ...
    const loadDiscPhoto = async () => {
      ...
      st.loading = true;
      try {
        const tex = await PIXI.Assets.load("/planets/Jupiter.png");
    ...
    const ticker = () => {
      ...
      // variable angular speed (2nd law), Jupiter at the focus, via the
      ...
      // Jupiter disc: true-scale world radius, clamped to a min on-screen
      ...
      // Soft halo behind the Jupiter photo: a few concentric faint rings so the
      ...
      for (const item of items) {
        ...
        // same scale as the Jupiter disc (computed in geo), clamped on screen
  ...
  return (
    <div ref={wrapRef} className="jms-fullscreen-wrap" role="dialog" aria-modal="true" aria-label={t("jupiter.system.fullscreenTitle")}>
      ...
      <div className="jms-top-bar">
        <div>
          <div className="jms-title">{t("jupiter.system.title")}</div>
          <div className="jms-sub">
            {t("jupiter.system.eyebrow", { count: moons.length || 0 })}
          </div>
        </div>
      ...
      {selected && (
        <div className="jms-card" onClick={(e) => e.stopPropagation()}>
          ...
          {selected.hasPhoto ? (
            <div className="jms-card-photo-wrap" style={{ overflow: "hidden" }}>
              ...
              <span className="jms-card-photo-cap">
                {["io", "europa", "ganymede", "callisto"].includes(selected.name.toLowerCase())
          ...
          <div className="jms-card-rows">
            <div className="jms-card-row"><span>{t("jupiter.tooltip.group")}</span><b>{t("jupiter.system." + selected.group)}</b></div>
