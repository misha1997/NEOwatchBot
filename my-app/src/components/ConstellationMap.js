// Interactive constellation / sky map.
//
// Self-contained: holds its own state (location, season, zoom, pan, hover) and
// renders the geolocation panel + polar sky SVG + constellation picker pills.
// Used twice from the Constellations page:
//   1. inline (default) — the map card on the page,
//   2. fullscreen=true — the same map in a fixed immersive overlay with a
//      close button, opened from a ⛶ button on the page (mirrors the Jupiter
//      moon-system fullscreen pattern).
//
...
export default function ConstellationMap() {
  ...
  // Fullscreen immersive overlay (mirrors the Jupiter moon-system ⛶ button:
  // the open button lives in the top-left corner of the inline map widget, and
  ...
  // Set document title (the inline page owns it; the fullscreen overlay does not
  // need to touch it — it's the same component instance, just a different view).
  useEffect(() => {
    if (showFs) return;
  ...
  useEffect(() => {
    ...
    const handleWheel = (e) => {
      ...
      const Ax = (e.clientX - rect.left) * (1500 / rect.width);
      const Ay = (e.clientY - rect.top) * (560 / rect.height);
      const zoomFactor = e.deltaY < 0 ? 1.15 : 0.85;

      setZoom((prevZoom) => {
        const nextZoom = Math.max(1.0, Math.min(60.0, prevZoom * zoomFactor));
        const ratio = 1 - nextZoom / prevZoom;
        setPanOffset((prevPan) => ({
          x: prevPan.x + (Ax - CX - prevPan.x) * ratio,
          y: prevPan.y + (Ay - CY - prevPan.y) * ratio
        }));
        return nextZoom;
      });
    };
  ...
  // Drag-to-pan pointer handlers (supports touch + mouse)
  ...
  const handleZoomIn = () => {
    setZoom((prev) => {
      const next = Math.min(60.0, prev * 1.3);
      setPanOffset((prevPan) => ({
        x: prevPan.x * (next / prev),
        y: prevPan.y * (next / prev)
      }));
      return next;
    });
  };
  ...
  const handleZoomOut = () => {
    setZoom((prev) => {
      const next = Math.max(1.0, prev * 0.75);
      setPanOffset((prevPan) => ({
        x: prevPan.x * (next / prev),
        y: prevPan.y * (next / prev)
      }));
  ...
  // Fullscreen-only lifecycle: lock body scroll + close on Escape.
  useEffect(() => {
    if (!showFs) return;
    const prevOverflow = document.body.style.overflow;
    document.body.style.overflow = "hidden";
    const onKey = (e) => {
      if (e.key === "Escape") setShowFs(false);
    };
  ...
  // --- shared render fragments (composed differently inline vs fullscreen) ---

  // Stellarium HUD tooltip (position: fixed, works in both layouts).
  const tooltipNode = hoveredObj && (
    <div
  ...
  // Zoom / reset button column (top-right corner; shared inline + fullscreen).
  const zoomButtons = (
    <div className="const-zoom-col">
      <button
        className="btn ghost const-zoom-btn"
        onClick={handleZoomIn}
        title={lang === "en" ? "Zoom In" : "Збільшити"}
        aria-label={lang === "en" ? "Zoom In" : "Збільшити"}
      >
        +
      </button>
      <button
        ...
        aria-label={lang === "en" ? "Zoom Out" : "Зменшити"}
      >
        −
      </button>
      <button
        ...
        aria-label={lang === "en" ? "Reset View" : "Скинути вигляд"}
      >
        ↺
      </button>
    </div>
  ...
  // Fullscreen-open ⛶ button — lives in the top-left corner of the inline map
  // widget (mirrors the Jupiter moon-system CTA). Icon-only, with a hover
  // tooltip describing what it opens.
  const openFsButton = (
    <button
      type="button"
      className="const-fs-cta"
      onClick={() => setShowFs(true)}
      aria-label={lang === "en" ? "Open fullscreen" : "На весь екран"}
    >
      <span className="const-fs-cta-ico">⛶</span>
      <span className="const-fs-cta-tip">
        {lang === "en" ? "Open fullscreen sky map" : "Відкрити карту неба на весь екран"}
      </span>
    </button>
  );

  // The polar sky SVG (or the loading placeholder).
  const mapSvgBlock = loading ? (
    <div className="const-map-loading">
    ...
    >
      ...
      <g transform={`translate(${panOffset.x}, ${panOffset.y}) translate(${CX}, ${CY}) scale(${zoom}) translate(${-CX}, ${-CY})`}>
        ...
        <circle cx={CX} cy={CY} r={R_HOR} fill="rgba(6, 12, 24, 0.82)" stroke="var(--border)" strokeWidth={(2 / zoom).toFixed(2)} />
          ...
          .map((s) => (
            <g key={`star-${s.id}`}>
              <circle
                ...
                cx={s.x.toFixed(1)} cy={s.y.toFixed(1)} r={(dotRadius(s.mag) / zoom).toFixed(2)}
                opacity={s.alt >= 0 ? (0.22 + (5.5 - s.mag) * 0.12).toFixed(2) : 0.08}
                style={{ animationDelay: `${(s.id % 20) * 0.2}s` }}
              />
              <circle
                className="const-hit"
                cx={s.x.toFixed(1)} cy={s.y.toFixed(1)} r={(10 / zoom).toFixed(2)}
              ...
              {visibleStars
                ...
                .map((s) => (
                  <g key={`major-star-${s.id}`}>
                    ...
                    <circle
                      className="const-hit"
                      cx={s.x.toFixed(1)} cy={s.y.toFixed(1)} r={(12 / zoom).toFixed(2)}
                      onMouseMove={(e) => { e.stopPropagation(); handleMouseMoveTooltip(e, "star", s); }}
                      onMouseLeave={handleMouseLeaveTooltip}
                    />
                  </g>
                ))}
              {c.x && c.y && (
                <text className="const-label" x={c.x.toFixed(1)} y={c.y.toFixed(1)} style={{ fontSize: (10.5 / zoom).toFixed(2) }}>
                  {lang === "en" ? c.name : c.uk}
                </text>
              )}
            </g>
  ...
  // Geolocation + season controls. Inline: a full bordered panel. Fullscreen:
  // a compact, borderless row that sits inside the floating bottom bar.
  const seasonPills = ["winter", "spring", "summer", "autumn"].map((s) => (
    <span
      key={s}
  ...
  const geoPanelNode = (
    <div className="geo-panel">
      ...
      <div
        className={`geo-status ${statusMsg.includes("Не вдалося") || statusMsg.includes("failed") ? "err" : (seasonFilter || geoInfo ? "ok" : "")}`}
        id="geoStatus"
      >
        {statusMsg}
      </div>
  ...
  // Compact controls for the fullscreen bottom bar (no panel chrome).
  const geoControlsCompact = (
    <>
      <button className="btn primary" onClick={handleAutoGeolocate} aria-label={lang === "en" ? "Show my sky" : "Показати моє небо"}>
        📍 {lang === "en" ? "My sky" : "Моє небо"}
      </button>
      {seasonPills}
    </>
  );

  // Constellation picker pills (the buttons themselves).
  const pillsInner = popularConstList.map((p) => {
